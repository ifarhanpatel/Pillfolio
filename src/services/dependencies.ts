  ...
  async pickImage(source: ImageSource): Promise<PickedImage | null> {
    ...
    if (source === 'camera') {
      ...
      try {
        ...
        const baseMessage = error instanceof Error ? error.message : 'Unable to launch camera.';
...
const defaultBackupBoundary: BackupBoundary = {
  async saveBackupFile(fileName: string, contents: string): Promise<string> {
    ...
    const backupDirectory = joinUri(baseDirectory, STORAGE_DIRS.backups);
    await FileSystem.makeDirectoryAsync(backupDirectory, { intermediates: true });

    const destinationUri = joinUri(backupDirectory, fileName.replace(/[^a-zA-Z0-9._-]/g, '_'));
  ...
  async shareFile(fileUri: string): Promise<void> {
    ...
    await Sharing.shareAsync(fileUri, {
      ...
      dialogTitle: 'Share Pillfolio backup',
      UTI: 'public.json',
    });
  },
};

export const createAppBoundaries = (overrides: Partial<AppBoundaries> = {}): AppBoundaries => {
  return {
    ...
    imageCompression: overrides.imageCompression ?? defaultImageCompressionBoundary,
    clock: overrides.clock ?? defaultClockBoundary,
    backup: overrides.backup ?? defaultBackupBoundary,
  };
};
